...
public class AccountManagerServiceImpl implements AccountManagerService {
    ...
    @Transactional(value = "salesTransactionManager")
    @Override
    public AccountManager partialUpdate(Integer accountmanagerId, Map<String, Object>accountManagerPatch) {
        LOGGER.debug("Partially Updating the AccountManager with id: {}", accountmanagerId);

        AccountManager accountManager = getById(accountmanagerId);

        try {
            ObjectReader accountManagerReader = this.objectMapper.readerForUpdating(accountManager);
            accountManager = accountManagerReader.readValue(this.objectMapper.writeValueAsString(accountManagerPatch));
        } catch (IOException ex) {
            LOGGER.debug("There was a problem in applying the patch: {}", accountManagerPatch, ex);
            throw new InvalidInputException("Could not apply patch",ex);
        }

        accountManager = update(accountManager);
